...
class NetworkQuery(object):
	...
	def test(self):
		sql = "select manufacturer from mac_manufacturer where mac_pre='94-94-26';"
		res = self.cur_psql.execute(sql)
		rows = self.cur_psql.fetchall()[0][0]
		print(rows)
	...
	def exec_port_scan(self, ip, tcp_ports='', udp_ports='', network=''):
		...
		if ret['scan']:
			...
			if scan_info_dict.get('vendor'):
				mac_info = scan_info_dict['vendor']
				mac_address = list(mac_info.keys())[0]
			else:
				try:
