...
class Telnet:
	...
	def connect(self, host, user, password, port=23, timeout=60 * 30, wait_time=3, user_pre=b"ogin:",
	            password_pre=b"assword:"):
		"""
		...
		try:
			self.tn = telnetlib.Telnet(host=host, port=port)
			time.sleep(1)
			...
			command_result = bytes(command_result, encoding="utf8")
			# message = json.dumps(self.message)
			# print("message:", type(message))
			self.websocker.send(self.message)
